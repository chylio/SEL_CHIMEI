...
/**
 * ModalPortal
 * - 用 portal 掛到 document.body，避免被外層 overflow/transform/z-index 擋住
 * - backdrop 用 button，確保可點擊關閉
 */
function ModalPortal({ children, onBackdrop }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] pointer-events-auto">
      ...
      />
      {/* Content wrapper */}
      <div className="relative z-[10000] w-full h-full flex items-center justify-center p-4 pointer-events-none">
    ...
    <ModalPortal onBackdrop={onClose}>
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onClose}
            className="flex-1 py-2.5 px-6 rounded-full border border-gray-200 text-sub-text hover:bg-gray-50 text-sm font-medium"
          >
            關閉
          </button>
        </div>
    ...
    <ModalPortal onBackdrop={onClose}>
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl overflow-hidden">
        ...
        <div className="relative h-28 rounded-2xl border border-gray-100 bg-gradient-to-br from-amber-50 via-white to-sky-50 mb-4 overflow-hidden">
          <div
            className={`absolute right-6 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full transition-all duration-500
          ...
          <div
            className={`absolute left-0 right-0 bottom-3 text-center text-xs font-semibold transition-all duration-400
...
function PlantGarden({ onClose }) {
  ...
  const [cells, setCells] = useState(() => Array.from({ length: 36 }, () => null))
