      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="bg-amber-50/60 border border-amber-100 rounded-xl p-5 mb-6">
          <div className="flex gap-2 mb-2">
            <span className="text-base font-semibold text-amber-700">📋 情境</span>
          </div>
          <p className="text-warm-text text-base leading-relaxed whitespace-pre-line">
            {question.scenario}
          </p>
        </div>
          ...
          {question.options.map((opt) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className={`text-base leading-relaxed ${isSelected ? 'text-warm-text font-medium' : 'text-sub-text'}`}>
                  {opt.text}
                </span>
              </label>
            )
...
function ResultSummaryCard({ result }) {
  // 防呆：避免 percentage 變成 undefined/NaN 導致 UI 空白
...
// ── 測驗結果 ──────────────────────────────────────────────────────────────────
function QuizResults({ answers, navigate, onRetest }) {
  const results = pharmacistQuizData.map((q) => {
    ...
    // 用 clampedScore 算百分比更穩定（避免負分或超過 maxScore 造成奇怪狀態）
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {/* 修正：不要 reload（會回到首頁），改用 onRetest 重置狀態並回到情境應用 */}
        <button
          onClick={onRetest}
...
export default function PharmacistQuizPage({ navigate }) {
  ...
  const toggleOption = (questionId, optionId) => {
    setAnswers((prev) => {
      ...
      return {
        ...prev,
        [questionId]: cur.includes(optionId) ? cur.filter((id) => id !== optionId) : [...cur, optionId],
      }
    })
  }
  ...
  if (stage === 'intro') return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      <button
        onClick={() => navigate('scenario')}
        className="flex items-center gap-1 text-sub-text text-sm mb-6 hover:text-warm-text transition-colors"
      >
        ← 返回情境應用
      </button>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      {currentAnswers.length > 0 && (() => {
        ...
        return (
          <div
            className={`mt-4 p-4 rounded-xl border text-sm font-medium ${passed
